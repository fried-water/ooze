...
BOOST_AUTO_TEST_CASE(bfs_traversal_converge) {
  const auto g = Graph<i32>(std::vector<std::vector<i32>>{{1, 3}, {2}, {}, {2}});
  check_range((std::vector{0, 1, 3, 2}), bfs_traversal(g, {0}));
  check_range((std::vector{1, 2}), bfs_traversal(g, {1}));
  check_range((std::vector{2}), bfs_traversal(g, {2}));
  check_range((std::vector{3, 2}), bfs_traversal(g, {3}));
}
