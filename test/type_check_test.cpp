...
void test_infer_header(const Env& e,
                       std::string_view expr,
                       std::string_view exp_header,
  ...
  const auto actual = inferred_header(expr_result.value(), std::move(active));
...
struct FunctionConverter {
  ...
  CheckedExpr operator()(const TypedExpr& expr) {
    const ast::Ident* i = std::get_if<ast::Ident>(&expr.v);

    return CheckedExpr{idx < overloads.size() && i && i->name == overloads[idx].name
                         ? overloads[idx++]
                         : knot::map<ast::ExprVariant<TypeID, EnvFunctionRef>>(expr.v, std::ref(*this)),
                       expr.ref};
  }
};
...
void test_or(const Env& e,
  ...
  const auto expected =
    parse_function(exp ? *exp : f)
      ...
      .map([&](TypedFunction f) {
        knot::preorder(f, [](Slice& ref) { ref = {}; });
        FunctionConverter converter{overloads};
        auto c = knot::map<CheckedFunction>(std::move(f), std::ref(converter));
        return c;
      });
...
BOOST_AUTO_TEST_CASE(infer_active) { test_infer_header(create_primative_env(), "a", "() -> _", {}, {"a"}); }
...
BOOST_AUTO_TEST_CASE(cp_nexted_tuple) {
  test_or(create_primative_env(),
          {},
          "(x: i32) -> _ = (x, (x, (x), x))",
...
BOOST_AUTO_TEST_CASE(cp_fn_identity) {
  test_or(create_primative_env(), {}, "(x: fn(i32) -> i32) -> _ = x", "(x: fn(i32) -> i32) -> fn(i32) -> i32 = x");
}

BOOST_AUTO_TEST_CASE(cp_return_fn) {
  Env e = create_primative_env();
  e.add_function("f", []() { return 1; });

  test_or(e, {{"f", 0, niso(I)}}, "() -> _ = f", "() -> fn() -> i32 = f");
}

BOOST_AUTO_TEST_CASE(cp_fn_call) {
...
BOOST_AUTO_TEST_CASE(cp_fn_assign) {
  ...
  test_or(e, {{"f", 0, niso(I)}}, "() -> i32 = { let x = f; x() }", "() -> i32 = { let x : fn() -> i32 = f; x() }");
}

BOOST_AUTO_TEST_CASE(cp_fn_tuple) {
  Env e = create_primative_env();
  e.add_function("f", []() { return 1; });
...
BOOST_AUTO_TEST_CASE(cp_return_arity_mismatch) {
  test_or_error(
    create_primative_env(), "() -> (i32, i32) = (1, 1, 1)", {{{19, 28}, "expected (i32, i32, i32), given (i32, i32)"}});
}
...
BOOST_AUTO_TEST_CASE(cp_unused_binding_tuple) {
  test_or_error(create_primative_env(),
                "((x): (i32)) -> _ = 1",
...
BOOST_AUTO_TEST_CASE(cp_unused_binding_self_assign) {
  test_or_error(create_primative_env(),
                "(x: i32) -> _ { let x = x; 1 }",
                {{{20, 21}, "unused binding 'x'", {"prefix with an _ to silence this error"}}});
}

BOOST_AUTO_TEST_CASE(cp_unused_binding_scope) {
  test_or_error(create_primative_env(),
                "() -> _ { { let x = 1; 1 } }",
...
BOOST_AUTO_TEST_CASE(cp_wrong_arg_count) {
  ...
  test_or_error(env, "() -> i32 = identity()", {{{20, 22}, "expected (), given (i32)"}});
}

BOOST_AUTO_TEST_CASE(cp_wrong_arg) {
  Env env = create_primative_env();
  env.add_function("identity", [](i32 x) { return x; });

  test_or_error(env, "(x: u32) -> i32 = identity(x)", {{{1, 2}, "expected u32, given i32"}});
}
...
BOOST_AUTO_TEST_CASE(cp_wrong_bind_count) {
  ...
  test_or_error(env, "(x: i32) -> i32 { let () = identity(x); x }", {{{27, 38}, "expected (), given i32"}});
}
...
BOOST_AUTO_TEST_CASE(cp_wrong_return_count) {
  ...
  test_or_error(env, "(x: i32) -> () = identity(x)", {{{17, 28}, "expected (), given i32"}});
}
...
BOOST_AUTO_TEST_CASE(cp_wrong_return_type) {
  ...
  test_or_error(e, "(x: i32) -> u32 = identity(x)", {{{18, 29}, "expected u32, given i32"}});
}
