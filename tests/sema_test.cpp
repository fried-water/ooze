...
auto run_nr(const NativeTypeInfo& types, std::string_view src) {
  return parse_and_name_resolution(parse_fn_expr, make_sv_array(src), types.names, {}, SrcID{0})
...
BOOST_AUTO_TEST_CASE(undefined_let) {
  ...
  const std::vector<ContextualError> exp = {{{SrcID{0}, {21, 22}}, "undefined type"}};
  check_eq("errors", exp, act);
}
...
BOOST_AUTO_TEST_CASE(identity) {
  check_sema(check_result(run_sema(basic_test_env(), "fn f(x: i32) -> i32 { x }")), {1});
}
...
BOOST_AUTO_TEST_CASE(fn_overload) {
  ...
  check_sema(check_result(run_sema(basic_test_env(fns), "fn g(x: i32, y: f32) { (f(x), f(y)) }")),
...
BOOST_AUTO_TEST_CASE(return_fn) {
  check_sema(check_result(run_sema(basic_test_env("f: fn() -> i32"), "fn g() { f }")), {0}); // f()
}
...
BOOST_AUTO_TEST_CASE(mod_fn) {
  check_sema(check_result(run_sema(basic_test_env(), "mod m { fn g() { 1 } } fn f() { m::g() }")), {0});
}

BOOST_AUTO_TEST_CASE(env_function_inside_mod) {
  check_sema(check_result(run_sema(basic_test_env("f: fn() -> i32"), "mod m { fn g() { f() } }")), {0});
}

BOOST_AUTO_TEST_CASE(partial) {
  TestEnv env = create_test_env({}, make_sv_array("f: fn() -> ()"));

  const std::string_view src1 = "fn g() { f() }";
  const std::string_view src2 = "fn h() { () }";
...
BOOST_AUTO_TEST_CASE(unresolved_fn) {
  ...
  check_eq("errors", exp_errors, check_error(run_sema(parse_fn_expr, {}, "(x) => x")));
}
...
BOOST_AUTO_TEST_CASE(borrow_partial) {
  const std::vector<ContextualError> exp_errors = {{{SrcID{1}, {1, 2}}, "unable to fully deduce type, deduced: _"}};
  check_eq("errors", exp_errors, check_error(run_sema(parse_fn_expr, {}, "(x) => { let _ = &x; () }")));
}

BOOST_AUTO_TEST_CASE(tuple_partial) {
  const std::vector<ContextualError> exp_errors = {{{SrcID{1}, {1, 2}}, "unable to fully deduce type, deduced: (_)"}};
  check_eq("errors", exp_errors, check_error(run_sema(parse_fn_expr, {}, "(x: (_)) => x")));

  const std::vector<ContextualError> exp_errors2 = {{{SrcID{1}, {1, 2}}, "unable to fully deduce type, deduced: (_)"}};
  check_eq("errors", exp_errors2, check_error(run_sema(parse_fn_expr, {}, "(x) -> (_) => x")));
}
...
BOOST_AUTO_TEST_CASE(fn) {
  ...
  const auto [pr, ast] = check_result(parse_fn_expr({}, SrcID{0}, srcs[0]));
  const Graph<ASTID> ident_graph = check_result(calculate_ident_graph(srcs, ast, as_span(pr.parsed)));
...
BOOST_AUTO_TEST_CASE(fn_unbound) {
  ...
  const auto [pr, ast] = check_result(parse_fn_expr({}, SrcID{0}, srcs[0]));
  const auto errors = check_error(calculate_ident_graph(srcs, ast, as_span(pr.parsed)));
  const std::vector<ContextualError> exp_errors = {{{SrcID{0}, {11, 12}}, "undeclared binding 'x'"}};
  BOOST_CHECK(exp_errors == errors);
}
...
BOOST_AUTO_TEST_CASE(call_global) {
  ...
  const auto srcs = make_sv_array(env_src, "fn g() { f() }");
...
BOOST_AUTO_TEST_CASE(overloaded) {
  const auto srcs = make_sv_array("fn f(Y) { () }"
                                  "fn f(X) { () }"
                                  "fn g() { f() }");
...
BOOST_AUTO_TEST_CASE(partial) {
  const auto srcs = make_sv_array("fn f() { g() }", "fn g() { f() }");
...
BOOST_AUTO_TEST_CASE(partial_error) {
  ...
  const auto srcs = make_sv_array("fn f() { h() }", "fn g() { () }");
  const auto [pr1, ast1] = check_result(parse({}, SrcID{0}, srcs[0]));
  const auto [pr2, ast] = check_result(parse(std::move(ast1), SrcID{1}, srcs[1]));
...
BOOST_AUTO_TEST_CASE(recursive) {
  const auto srcs = make_sv_array("fn f() { f() }");
  const auto [pr, ast] = check_result(parse({}, SrcID{0}, srcs[0]));
  const Graph<ASTID> ident_graph = check_result(calculate_ident_graph(srcs, ast, pr.parsed));
...
BOOST_AUTO_TEST_CASE(mod_fn) {
  const auto srcs = make_sv_array("mod m { fn g() { 1 } } fn f() { m::g() }");
  const auto [pr, ast] = check_result(parse({}, SrcID{0}, srcs[0]));
...
BOOST_AUTO_TEST_CASE(inaccessible_inner_mod_fn) {
  const auto srcs = make_sv_array("mod m { fn g() { 1 } } fn f() -> () { g() }");
  const auto [pr, ast] = check_result(parse({}, SrcID{0}, srcs[0]));
  const std::vector<ContextualError> exp_errors = {{{SrcID{0}, {38, 39}}, "undeclared binding 'g'"}};
...
BOOST_AUTO_TEST_CASE(inaccessible_outer_mod_fn) {
  const auto srcs = make_sv_array("fn f() { 1 } mod m { fn g() { f() } } ");
  const auto [pr, ast] = check_result(parse({}, SrcID{0}, srcs[0]));
  const std::vector<ContextualError> exp_errors = {{{SrcID{0}, {30, 31}}, "undeclared binding 'f'"}};
  check_eq("ig_errors", exp_errors, check_error(calculate_ident_graph(srcs, ast, pr.parsed)));
}

// TODO change error to specify can't find a module
BOOST_AUTO_TEST_CASE(nonexistant_mod) {
  const auto srcs = make_sv_array("mod m { fn g() { 1 } } fn f() -> () { n::g() }");
  const auto [pr, ast] = check_result(parse({}, SrcID{0}, srcs[0]));
  const std::vector<ContextualError> exp_errors = {{{SrcID{0}, {38, 42}}, "undeclared binding 'n::g'"}};
  check_eq("ig_errors", exp_errors, check_error(calculate_ident_graph(srcs, ast, pr.parsed)));
}

BOOST_AUTO_TEST_CASE(nonexistant_fn_in_mod) {
  const auto srcs = make_sv_array("mod m { fn g() { 1 } } fn f() -> () { m::h() }");
  const auto [pr, ast] = check_result(parse({}, SrcID{0}, srcs[0]));
  const std::vector<ContextualError> exp_errors = {{{SrcID{0}, {41, 42}}, "undeclared binding 'h'"}};
  check_eq("ig_errors", exp_errors, check_error(calculate_ident_graph(srcs, ast, pr.parsed)));
}

BOOST_AUTO_TEST_CASE(nonexistant_fn_as_mod) {
  const auto srcs = make_sv_array("fn g() { 1 } fn f() -> () { g::f() }");
  const auto [pr, ast] = check_result(parse({}, SrcID{0}, srcs[0]));
  const std::vector<ContextualError> exp_errors = {{{SrcID{0}, {28, 32}}, "undeclared binding 'g::f'"}};
  check_eq("ig_errors", exp_errors, check_error(calculate_ident_graph(srcs, ast, pr.parsed)));
}
