...
template <typename Parser>
auto frontend(
  Parser p, Span<std::string_view> srcs, const NativeTypeInfo& native_types, AST ast, Span<ASTID> global_imports = {}) {
  assert(srcs.size() == 2);
  ...
  return parse_and_name_resolution(p, srcs, native_types.names, std::move(ast), SrcID{1})
    .and_then([&](auto roots, AST ast) {
      return sema(srcs, tc, native_types, std::move(ast), as_span(roots), global_imports);
    });
}
